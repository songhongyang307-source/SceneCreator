/*==============================================================================
   ...
   Billboard  shader  [shader_billboard.cpp]
														 Author : Youhei Sato
														 Date   : 2025/11/14
--------------------------------------------------------------------------------
...
bool ShaderBillboard_Initialize()
{
	...
	if (FAILED(hr)) {
		hal::dout << "ShaderBillboard_Initialize() : 頂点シェーダー作成失敗" << std::endl;
	...
	buffer_desc.ByteWidth = sizeof(UVParameter); // size of buffer
	...
	if (FAILED(hr)) {
		hal::dout << "ShaderBillboard_Initialize() : Pixelシェーダー作成失敗" << std::endl;
		return false;
	}
